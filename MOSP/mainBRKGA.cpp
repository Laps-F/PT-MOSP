...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "Uso: ./mainBRKGA <caminho_para_instancia> [--INST <instance>] [--OUTDIR <outdir>] [--GENS <generations>] [--TIME_LIMIT <seconds>] [--PRINT_EVERY <print_every>] [--THREAD_USED <threads>] [--VERBOSE <0|1>]" << std::endl;
        return 1;
    }
    ...
    MospDecoder decoder(m, n, matriz, start_time, time_limit);
    ...
    if (verbose) {
        std::cout << "BRKGA START: instance='" << fn << "' m=" << m << " n=" << n
                  ...
                  << " gens=" << max_generations << " time_limit=" << time_limit << "s"
    ...
    while (gen < max_generations) {
        ...
        if (time_limit > 0.0 && elapsed.count() >= time_limit) {
            if (verbose) {
                std::cout << "Time limit reached at generation " << gen << std::endl;
            }
            break;
        }

        if (verbose && ((gen) % print_every == 0 || gen == max_generations)) {
            // double current_best = algorithm.getBestFitness();
            auto gen_end = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> gen_elapsed = gen_end - gen_start;
            // std::cout << "GEN " << gen << "/" << max_generations
            //           << " | best=" << current_best
