...
# --- Settings ---
RESULTS_FOLDER = "Results/SCOOP"
GOOGLE_SHEET_NAME = "Results_SCOOP"  # Changed sheet name
...
def calculate_subset_statistics(subsets_by_size, best_data):
    """
...
def main():
    ...
    subset_stats = calculate_subset_statistics(subsets, bests)
    ...
    df_all = sort_dataframe(df_all, sort_by_size_first=False)
    df_mean = sort_dataframe(df_mean, sort_by_size_first=False)
    df_best = sort_dataframe(df_best, sort_by_size_first=False)
    
    # Nova ordenação (por subset primeiro)
    df_subset = sort_dataframe(df_subset, sort_by_size_first=False)
